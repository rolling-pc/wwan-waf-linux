...
// const QString K_SWITCH_PCIE_EDL =
...
#endif
bool FirmwareFlashImplQcomPCIE::WaitEdlDevice() {
...
#else
    auto edl_tty = dev_factory->CreateDevice("wwan", {});
    edl_tty->AddEnv("DEVNAME", "/dev/wwan0firehose0");
#endif
...
FirmwareFlashImplQcomPCIE::FirmwareFlashImplQcomPCIE(ModemType modem_type)
    : FirmwareFlashImplQcomPCIE(modem_type, DeviceFactory::Create()) {}

FirmwareFlashImplQcomPCIE::FirmwareFlashImplQcomPCIE(
    ModemType modem_type, std::unique_ptr<DeviceFactory> dev_factory)
    : FirmwareFlashImplQcomPCIE(modem_type, std::move(dev_factory),
                               Fastboot::Create()) {}
...
FirmwareFlashImplQcomPCIE::FirmwareFlashImplQcomPCIE(
    ...
    modem = this->dev_factory->CreateModem("pci", K_MODEM_MAP);
    fastboot_device = this->dev_factory->CreateDevice("wwan", K_FASTBOOT_MAP);
#ifdef _IS_WINDOWS_
    edl_device = this->dev_factory->CreateDevice("usb", K_RECOVER_MAP);
#else
    edl_device = this->dev_factory->CreateDevice("wwan", {});
    edl_device->AddEnv("DEVNAME", "/dev/wwan0firehose0");
#endif
}

bool FirmwareFlashImplQcomPCIE ::SwitchToFastbootMode() {
...
//     FMTLOG_DEBUG("Switch To EDL: Qbhiserver.exe --forceedl 0");
//     bool ok = RunQBhiServer("--forceedl 0");
//     if (!ok) return false;
// #ifdef _IS_WINDOWS_
//     Sleep(3000);
// #endif
//     return WaitEdlDevice();
// }

bool FirmwareFlashImplQcomPCIE::PcieReset() {
    return true;
}

FirmwareFlash::FlashMode FirmwareFlashImplQcomPCIE::QueryFlashMode() {
...
void FirmwareFlashImplQcomPCIE::EdlConfig(const QString& fw_path,
...
bool FirmwareFlashImplQcomPCIE::EdlFlash() {
    ...
    FlashImageComplete(0, "Start flashing...");
...
bool FirmwareFlashImplQcomPCIE::EdlRead(const QString& save_path,
...
bool FirmwareFlashImplQcomPCIE::EdlEraseAll() {
    FMTLOG_ERROR("This feature is not supported yet.");
    return false;
}

bool FirmwareFlashImplQcomPCIE::EdlErase(int start_addr, int partition_size) {
...
void FirmwareFlashImplQcomPCIE::HandleProcessOutput(QString output) {
...
void FirmwareFlashImplQcomPCIE::HandleProcessFinished(
