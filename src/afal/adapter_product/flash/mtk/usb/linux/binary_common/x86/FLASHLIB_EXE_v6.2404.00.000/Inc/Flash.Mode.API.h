...
//! Effects: set the library env.
//! Change the lib env settings mainly for brom and DA.
//!
//! Parameters:
//!    hs: the session handle.
//!    runtime_env_xml: xml string contained formatted lib env settings. UTF-8 string.
//!
//! Returns:  E_OK if success, or other status indicate specified error.
//! 
LIBEX_EXPORT ZHRESULT LIB_API flashtool_set_runtime_env(HSESSION hs, const char *runtime_env_xml);
