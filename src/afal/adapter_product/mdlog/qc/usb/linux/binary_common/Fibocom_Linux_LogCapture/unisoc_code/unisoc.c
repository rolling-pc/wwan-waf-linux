...
static void diag_pkt_input(struct diag_pkt *diag_pkt, const uint8_t *pSrc, size_t size) {
    ...
    if (diag_pkt->last_7d) {
        if (diag_pkt->start_7e == 0 ||  diag_pkt->pkt_len) {
            ...
            diag_pkt->errors++;
            if (diag_pkt_debug) LogInfo("should get 0x7e here 1 !\n");
        }
    ...
    for (i = 0; i < size; i++) {
        if (*pSrc == 0x7d) {
            ...
            i++;
            if (i == size) {
                if (diag_pkt_debug) LogInfo("last_7d\n");
                diag_pkt->last_7d = 1;
                break;
            }
            ...
            if (diag_pkt->start_7e == 0 ||  diag_pkt->pkt_len) {
                ...
                diag_pkt->errors++;
                if (diag_pkt_debug) LogInfo("should get 0x7e here 2 !\n");
            }
        }
        else if (*pSrc == 0x7E) {
            ...
            if (diag_pkt->pkt_len >= diag_pkt->min_pkt_len) {
                ...
                if (diag_pkt->pkt_len > max_pkt_size) {
                    ...
                    if (diag_pkt_debug) LogInfo("max_pkt_size %zd\n", max_pkt_size);
                }
                ...
                diag_pkt->errors++;
                if (diag_pkt_debug) LogInfo("get 7e7e here!\n");
                diag_pkt->pkt_len = 1;
            }
            else {
                diag_pkt->errors++;
                if (diag_pkt_debug) LogInfo("two short pkt len %zd!\n", diag_pkt->pkt_len);
                diag_pkt->pkt_len = 0;
            }
        }
        else if (diag_pkt->start_7e && diag_pkt->pkt_len == 0) {
            diag_pkt->errors++;
            if (diag_pkt_debug) LogInfo("should get 0x7e here 3 !\n");
            pSrc++;
        }
        else {
            diag_pkt->buf[diag_pkt->pkt_len++] = (*pSrc++);
            if (diag_pkt->pkt_len > diag_pkt->max_pkt_len) {
                diag_pkt->errors++;
                if (diag_pkt_debug) LogInfo("two long pkt len %zd!\n", diag_pkt->pkt_len);
                diag_pkt->pkt_len = 0;
            }
        }
    }
...
unisco_send_at_command("AT+SPLOGLEVEL=1,3,\"60FBF8700FFD0008FFF0000000000000\",\"E7F302E708FFFFFFFFFF70E7CCBFF1C000FF030CFDFFFFFF8F710000000000E3\"");
    ...
    if (g_is_unisoc_chip == 1)
    {
        ...
        while (ulog_exit_requested != 0)
        {
            ...
            {
                // query modem UE time
                request.len         = ulog_le16(DIAG_HDR_LEN);
                request.type        = DIAG_SYSTEM_F;
                request.subtype     = 0x11;
                request.sn          = ulog_le32(0);

                lpBuf[0] = 0x7E;
                memcpy(lpBuf+1, &request, DIAG_HDR_LEN);
                lpBuf[DIAG_HDR_LEN+1] = 0x7E;

                if (ulog_poll_write(fd, lpBuf, DIAG_HDR_LEN+2, 0) <= 0) { return -1; }
            }
    ...
    else if (g_is_unisoc_chip == 2)
    {
        const uint8_t at_prepare[] =  {0x7e, 0x00, 0x00, 0x00, 0x00, 0x0c, 0x00, 0x62, 0x60, 0x00, 0x00,
                                        0x00, 0x00, 0x7e};
        if (ulog_poll_write(fd, at_prepare, sizeof(at_prepare), 0) <= 0) { return -1; }
        usleep(1);
        ...
        if (ulog_poll_write(fd, at_armlog, sizeof(at_armlog), 0) <= 0) { return -1; }
        ...
        if (ulog_poll_write(fd, at_splogping, sizeof(at_splogping), 0) <= 0) { return -1; }
        usleep(1);
