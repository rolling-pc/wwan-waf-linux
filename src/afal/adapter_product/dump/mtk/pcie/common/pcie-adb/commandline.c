...
void version(FILE * out) {
    fprintf(out, "Android Debug Bridge version %d.%d.%d.%d\n",
         ADB_VERSION_MAJOR, ADB_VERSION_MINOR, ADB_SERVER_VERSION, ADB_BUILD_VERSION);
}
...
void help()
{
    ...
    fprintf(stderr,
        ...
        "                                 be an absolute path.\n"
        " -H                            - Name of pcie-adb server host (default: localhost)\n"
        " -P                            - Port of pcie-adb server (default: 5039)\n"
        ...
        "  ADB_PCIE_DATA_TRACE          - Print the raw data of each packet, only available when ADB_TRACE is set.\n"
        );
}
