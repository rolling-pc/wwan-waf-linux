...
void print_packet(const char *label, apacket *p)
{
    ...
    char packet[1024];
    packet[1023] = 0;
    snprintf(packet, 1023, "%s: %s %08x %08x %04x \"",
             label, tag, p->msg.arg0, p->msg.arg1, p->msg.data_length);
    // End - Jeffrey
    ...
    while(count-- > 0){
        if((*x >= ' ') && (*x < 127)) {
            // fputc(*x, stderr);
            snprintf(packet, 1023, "%s%c", packet, *x);
        } else {
            // fputc('.', stderr);
            snprintf(packet, 1023, "%s%c", packet, '.');
        }
        x++;
    }
    // fputs(tag, stderr);
    snprintf(packet, 1023, "%s%s", packet, tag);
    DR(packet);
    // End - Jeffrey
}
#endif
    ...
    if (cp) {
        *cp++ = 0;
	    ...
	    static const char *dev_info[] = {
            ...
            if (strcmp(t->device, PCIE_SERIAL_UNKNOW)) {
                snprintf(dev_serial, sizeof(dev_serial), "%s%s%s", t->device, t->product, t->model);
                qual_overwrite(&t->serial, dev_serial);
            }
#else
...
void start_logging(void)
{
    ...
    char *log_folder = "./adb_log";
    struct tm now;
    time_t t;
    int ret;

    ret = access(log_folder, F_OK);
    if (ret == -1) {
        // Begin - Jeffrey
        ...
        if (errno = -ENOENT) {
            // Begin - Jeffrey
            if (-1 == adb_mkdir(log_folder, 0775))
            {
                fprintf(stderr, "can not create log folder, err:%s\n", strerror(errno));
            }
            else
            {
                fprintf(stderr, "created log folder\n");
            }
            // End - Jeffrey
        } else {
            fprintf(stderr, "Err: %s, can not access log folder\n", strerror(errno));
            return;
        }
    }
    ...
    snprintf(file_path, sizeof(file_path), "%s/%s", log_folder, path);
    fd = unix_open(file_path, O_WRONLY | O_CREAT | O_TRUNC, 0640);
    if(fd < 0) {
...
int launch_server(int server_port)
{
    ...
    if (pid == 0) {
        ...
        adb_close(fd[0]);
        // Begin - Jeffrey
        // We send "OK\n" to the parent process here. Because the original code hijacked the stderr which will cause a lot of problems 
...
#else
        // Begin - Jeffrey
        // We alreay send it in launch_server after pipe
        // fprintf(stderr, "OK\n");
        // End - Jeffrey
#endif
        // Begin - Jeffrrey
        // THis function was already called if the ADB_TRACING is 1 
        // start_logging();
        // End - Jeffrey
    }
